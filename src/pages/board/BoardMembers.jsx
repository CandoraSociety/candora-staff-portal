...
import { Plus, Mail, Phone, Trash2, CheckCircle, Circle, User, Pencil } from "lucide-react";
...
export default function BoardMembers() {
  ...
  const handleSave = async (e) => {
    ...
    setSaving(true);
    if (editingMember) {
      const updated = await base44.entities.BoardMember.update(editingMember.id, form);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setEditingMember(null); setForm({ full_name: "", email: "", role: "Director", phone: "", term_start: "", term_end: "", bio: "", committee: "" }); setShowForm(!showForm); }} className="flex items-center gap-2 bg-primary text-primary-foreground px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition">
      ...
      {showForm && (
        <form onSubmit={handleSave} className="bg-card border border-border rounded-xl p-5 mb-6">
          <h3 className="font-semibold mb-4">{editingMember ? `Edit ${editingMember.full_name}` : "New Board Member"}</h3>
          ...
          <div className="flex gap-3 mt-4">
            <button type="button" onClick={() => { setShowForm(false); setEditingMember(null); }} className="flex-1 border border-border rounded-lg py-2 text-sm hover:bg-muted transition">Cancel</button>
            <button type="submit" disabled={saving} className="flex-1 bg-primary text-primary-foreground rounded-lg py-2 text-sm font-medium hover:opacity-90 transition disabled:opacity-60">{saving ? "Saving..." : editingMember ? "Save Changes" : "Add Member"}</button>
          </div>
        </form>
      )}
                    ...
                    <div key={member.id} className="bg-card border border-border rounded-xl p-4 hover:shadow-sm transition group">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                          <button onClick={() => handleEdit(member)} className="p-1.5 text-muted-foreground hover:text-foreground rounded" title="Edit member"><Pencil size={14} /></button>
