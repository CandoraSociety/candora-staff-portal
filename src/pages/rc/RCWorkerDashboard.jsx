...
// Personal caseworker dashboard — shows client visits assigned to the signed-in
// caseworker, pending until they complete the visit record.
export default function RCWorkerDashboard() {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-4 max-w-3xl mx-auto">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Client visits assigned to you — complete each visit record to add it to the client's service history</p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">Pending Visits ({pending.length})</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {isLoading ? <p className="text-sm text-muted-foreground text-center py-6">Loading...</p> :
            pending.length === 0 ? (
              ...
              pending.map(v => <PendingVisitCard key={v.id} visit={v} onCompleted={onCompleted} />)
            )}
        </CardContent>
      </Card>
      ...
      {completed.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-base">Completed Visits ({completed.length})</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {completed.map(v => (
              <div key={v.id} className="p-3 rounded-md border border-border/50">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-sm font-medium text-foreground">{v.client_name} — {VISIT_TYPE_LABELS[v.visit_type] || v.visit_type}</p>
