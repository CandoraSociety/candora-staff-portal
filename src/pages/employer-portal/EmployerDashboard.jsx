...
export default function EmployerDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [ps, subs] = await Promise.all([
        base44.entities.WorkExposurePlacement.filter({ employer_id: employerProfile.id }),
        base44.entities.WorkExposureHoursSubmission.filter({ employer_id: employerProfile.id }),
      ]);
  ...
  const activePlacements = placements; // already excludes cancelled

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Welcome, {employerProfile?.contact_name?.split(' ')[0] || 'Employer'}</h1>
          <p className="text-sm text-slate-600">Submit work exposure hours for your placement participants.</p>
        </div>
        <Button onClick={() => setShowForm(true)} disabled={activePlacements.length === 0}>
      ...
      {activePlacements.length === 0 && (
        <Card>
          <CardContent className="py-10 text-center text-sm text-slate-500">
            No active work exposure participants are assigned to your company yet.
            A Candora career counsellor will link participants to your business once a placement begins.
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <InfoRow icon={Building2} label="Company" value={employerProfile?.name} />
          <InfoRow icon={Briefcase} label="Industry" value={employerProfile?.industry} />
          <InfoRow icon={MapPin} label="Address" value={employerProfile?.address} />
          <InfoRow icon={Phone} label="Phone" value={employerProfile?.contact_phone} />
          <InfoRow icon={Mail} label="Login email" value={employerProfile?.contact_email} />
          <InfoRow icon={Building2} label="Contact" value={employerProfile?.contact_name} />
        </CardContent>
      </Card>
    </div>
  );
