...
export default function CommunitySessions() {
  ...
  const { data: sessions = [], isLoading } = useQuery({ queryKey: ['community-sessions'], queryFn: () => base44.entities.CommunitySession.list('-session_date', 200) });
  const { data: programs = [] } = useQuery({ queryKey: ['community-programs'], queryFn: () => base44.entities.CommunityProgram.list('name', 200) });

  const filtered = sessions.filter(s => {
    const matchSearch = (s.title || '').toLowerCase().includes(search.toLowerCase()) || (s.facilitator_name || '').toLowerCase().includes(search.toLowerCase()) || (s.program_name || '').toLowerCase().includes(search.toLowerCase());
    const matchProgram = programFilter === 'all' || s.program_id === programFilter;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-heading font-bold text-foreground">Sessions</h1><p className="text-muted-foreground text-sm mt-1">Schedule sessions across all community programs</p></div>
        <Button onClick={openNew}><Plus className="h-4 w-4" /> New Session</Button>
      </div>

      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search sessions..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" /></div>
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="w-full sm:w-36"><SelectValue placeholder="All statuses" /></SelectTrigger><SelectContent><SelectItem value="all">All statuses</SelectItem>{SESSION_STATUS_OPTIONS.map(s => <SelectItem key={s.value} value={s.value}>{s.label}</SelectItem>)}</SelectContent></Select>
      </div>
            ...
            return (
              <Card key={s.id} className="hover:shadow-md transition-shadow"><CardContent className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="min-w-0 flex-1"><p className="font-medium text-sm text-foreground truncate">{s.title || 'Untitled Session'}</p><p className="text-xs text-muted-foreground">{s.program_name}</p></div>
