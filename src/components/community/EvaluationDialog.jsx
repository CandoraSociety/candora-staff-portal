...
import { ENGAGEMENT_OPTIONS, PARTICIPATION_OPTIONS } from '@/lib/communityConstants';

const EMPTY = { participant_id: '', participant_name: '', program_id: '', program_name: '', session_id: '', session_title: '', evaluation_date: '', evaluator_name: '', evaluator_email: '', engagement: 'moderate', participation: 'met', progress_observed: '', areas_for_improvement: '', overall_rating: 3, notes: '' };
...
export default function EvaluationDialog({ open, onOpenChange, evaluation, presetParticipantId, presetParticipantName, onSaved }) {
  ...
  const { data: participants = [] } = useQuery({ queryKey: ['community-participants'], queryFn: () => base44.entities.CommunityParticipant.list('-created_date', 500), enabled: open });
  const { data: sessions = [] } = useQuery({ queryKey: ['community-sessions'], queryFn: () => base44.entities.CommunitySession.list('-session_date', 200), enabled: open && !!form.program_id });
  ...
  const handleSessionSelect = (id) => {
    ...
    update('session_title', s?.title || '');
  };

  const programSessions = sessions.filter(s => s.program_id === form.program_id);

  const handleSave = async () => {
    if (!form.participant_id || !form.program_id || !form.evaluation_date) { toast({ title: 'Participant, program, and date are required', variant: 'destructive' }); return; }
    setSaving(true);
    try {
      if (evaluation) await base44.entities.CommunityEvaluation.update(evaluation.id, form);
      else await base44.entities.CommunityEvaluation.create(form);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{evaluation ? 'Edit Evaluation' : 'New Evaluation'}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5 col-span-2"><Label>Session (optional)</Label><Select value={form.session_id || ''} onValueChange={handleSessionSelect}><SelectTrigger><SelectValue placeholder="General evaluation (no specific session)" /></SelectTrigger><SelectContent>{programSessions.map(s => <SelectItem key={s.id} value={s.id}>{s.title || 'Untitled'} — {s.session_date ? new Date(s.session_date).toLocaleDateString() : ''}</SelectItem>)}</SelectContent></Select></div>
          ...
          <div className="space-y-1.5"><Label>Evaluator Name</Label><Input value={form.evaluator_name || ''} onChange={(e) => update('evaluator_name', e.target.value)} /></div>
          <div className="space-y-1.5"><Label>Engagement</Label><Select value={form.engagement || 'moderate'} onValueChange={(v) => update('engagement', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{ENGAGEMENT_OPTIONS.map(e => <SelectItem key={e.value} value={e.value}>{e.label}</SelectItem>)}</SelectContent></Select></div>
          <div className="space-y-1.5"><Label>Participation</Label><Select value={form.participation || 'met'} onValueChange={(v) => update('participation', v)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{PARTICIPATION_OPTIONS.map(p => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}</SelectContent></Select></div>
