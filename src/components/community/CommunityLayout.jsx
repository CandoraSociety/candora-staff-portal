...
import { LayoutDashboard, FolderTree, Users, CalendarDays, Star, Menu, X, ArrowLeft, Heart } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useOrgSettings } from '@/lib/useOrgSettings';
import EAFloatingWidget from '@/components/ed/EAFloatingWidget';

const NAV_ITEMS = [
  { path: '/community', label: 'Dashboard', icon: LayoutDashboard },
  { path: '/community/programs', label: 'Programs', icon: FolderTree },
  { path: '/community/participants', label: 'Participants', icon: Users },
  { path: '/community/sessions', label: 'Sessions', icon: CalendarDays },
  { path: '/community/evaluations', label: 'Evaluations', icon: Star },
];
...
export default function CommunityLayout() {
  ...
  const isActive = (path) => path === '/community' ? location.pathname === '/community' : location.pathname.startsWith(path);
      ...
      <header className="sticky top-0 z-40 bg-sidebar border-b border-sidebar-border">
        <div className="flex items-center justify-between px-4 lg:px-6 h-14">
          <div className="flex items-center gap-3">
            ...
            {logoUrl ? <img src={logoUrl} alt={orgName} className="h-8 w-auto object-contain" /> : <div className="h-8 w-8 rounded-lg bg-sidebar-primary flex items-center justify-center"><Heart className="h-4 w-4 text-sidebar-primary-foreground" /></div>}
            <div><h1 className="text-sidebar-foreground font-display font-bold text-sm leading-tight">Community Programs</h1><p className="text-sidebar-foreground/60 text-xs leading-tight">Occasional & volunteer-run programs</p></div>
          </div>
