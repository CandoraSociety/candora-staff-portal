  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      >
        <button
          onClick={() => setMobileOpen(false)}
          className="absolute top-3 right-3 p-1 rounded-md hover:opacity-70"
          style={{ color: "hsl(45,60%,88%)" }}
        >
          <X className="w-4 h-4" />
        </button>
      ...
      <main className="flex-1 overflow-y-auto bg-background">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b md:hidden" style={{ background: "hsl(230,70%,10%)" }}>
          <button onClick={() => setMobileOpen(true)} style={{ color: "hsl(45,60%,88%)" }}>
            <Menu className="w-5 h-5" />
          </button>
          <p className="text-sm font-bold" style={{ color: "hsl(45,92%,53%)" }}>Executive Assistant Portal</p>
        </div>
        <Outlet />
      </main>
    </div>
