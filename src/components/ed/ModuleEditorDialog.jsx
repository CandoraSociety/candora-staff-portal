...
import { Plus, Trash2, Upload, Loader2, GripVertical, ChevronUp, ChevronDown, Link2, FileText, Video, Presentation } from "lucide-react";
...
const emptyModule = {
  ...
  content_html: "", duration_minutes: 0, difficulty: "beginner",
  tags: [], file_attachments: [], learning_objectives: [], quiz_questions: [],
  status: "draft", version: 1,
};

export default function ModuleEditorDialog({ open, onClose, onSave, editingModule }) {
  ...
  useEffect(() => {
    if (editingModule) {
      setForm({ ...emptyModule, ...editingModule });
    } else {
      setForm(emptyModule);
    }
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 mt-3 pr-1">
            ...
            <TabsContent value="content" className="mt-0">
              <div className="mb-2 text-xs text-muted-foreground">Write the lesson content below. You can include text, images, links, and embedded videos.</div>
            ...
            <TabsContent value="files" className="space-y-3 mt-0">
              <div>
                <Label className="mb-2 block text-xs">Attach slides, videos, documents, or images</Label>
            ...
            <TabsContent value="objectives" className="space-y-2 mt-0">
              ...
              <p className="text-xs text-muted-foreground">What will the learner be able to do after completing this module?</p>
              {form.learning_objectives.length === 0 ? (
                <p className="text-sm text-muted-foreground/60 italic py-4 text-center">No objectives added yet.</p>
              ) : (
        ...
        <DialogFooter className="mt-3 border-t pt-3">
          <div className="flex items-center justify-between w-full">
            <div className="text-xs text-muted-foreground">
              {form.file_attachments.length} file(s) · {form.quiz_questions.length} quiz Q(s) · {form.learning_objectives.length} objective(s)
            </div>
            <div className="flex gap-2">
