...
export default function EARecoveryScanner({ onRestore, headerMode }) {
      ...
      <div className="relative" onClick={e => e.stopPropagation()}>
        ...
        {open && (
          <>
            ...
            <div
              className="absolute right-0 top-9 z-50 rounded-xl shadow-2xl p-3 w-72 space-y-2"
            ...
            >
              ...
              {results !== null && results.length === 0 && (
                <p className="text-xs" style={{ color: "hsl(230,30%,55%)" }}>
                  No saved conversations found in your browser storage.
                </p>
              )}
              ...
              {results !== null && results.map((r) => (
                <div key={r.key} className="rounded-lg p-2.5 space-y-1.5" style={{ background: "hsl(230,55%,14%)", border: "1px solid hsl(230,45%,22%)" }}>
                  <p className="text-[10px] font-mono break-all" style={{ color: "hsl(230,30%,55%)" }}>{r.key}</p>
                  <p className="text-xs" style={{ color: "hsl(45,50%,80%)" }}>
                    {r.count} messages · last: "{r.messages[r.messages.length - 1]?.content?.slice(0, 50)}..."
                  </p>
                  <button
