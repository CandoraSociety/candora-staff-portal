...
export default function PlacementListTab({ clients, type }) {
  ...
  const grouped = STATUS_GROUPS.map(status => ({
    status,
    clients: filtered.filter(c => getStatus(c) === status),
  }));
  ...
  return (
    <div className="space-y-6">
      {grouped.map(({ status, clients: groupClients }) => (
        <div key={status} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-2.5 border-b border-slate-200 bg-slate-50">
            ...
            <span className="text-sm text-slate-500">{groupClients.length} client{groupClients.length !== 1 ? "s" : ""}</span>
          </div>
                ...
                <tr>
                  ...
                  {isInternal ? (
                    ...
                    <>
                      ...
                      <th className="text-left px-3 py-2 font-semibold text-slate-500 whitespace-nowrap">Employer</th>
                      <th className="text-left px-3 py-2 font-semibold text-slate-500 whitespace-nowrap">Job Title</th>
                      <th className="text-left px-3 py-2 font-semibold text-slate-500 whitespace-nowrap">Start Date</th>
                    </>
                  )}
