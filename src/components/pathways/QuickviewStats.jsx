...
import { Users, CheckCircle, TrendingUp, Briefcase, DollarSign } from 'lucide-react';
...
export default function QuickviewStats() {
  ...
  // Build worker list from PathwaysStaff, plus any workers assigned to clients but not in staff list
  const staffEmails = new Set(staff.map(s => s.email.toLowerCase()));
  const byWorker = staff.map(s => ({
    name: s.name,
    email: s.email,
    role: s.role,
    count: clients.filter(c => c.assigned_worker?.toLowerCase() === s.email.toLowerCase()).length,
    active: clients.filter(c => c.assigned_worker?.toLowerCase() === s.email.toLowerCase() && c.status === 'active').length,
  }));

  const additionalEmails = [...new Set(clients.map(c => c.assigned_worker).filter(Boolean))]
    .filter(email => !staffEmails.has(email.toLowerCase()));
  const additionalWorkers = additionalEmails.map(email => {
    const c = clients.find(c => c.assigned_worker?.toLowerCase() === email.toLowerCase());
    return {
      name: c?.assigned_worker_name || email,
      email,
      role: null,
      count: clients.filter(c => c.assigned_worker?.toLowerCase() === email.toLowerCase()).length,
      active: clients.filter(c => c.assigned_worker?.toLowerCase() === email.toLowerCase() && c.status === 'active').length,
    };
  });

  const allWorkers = [...byWorker, ...additionalWorkers];
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Caseload by Worker</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {w.role && (
                      <Badge variant="outline" className="text-xs">
                        {STAFF_ROLE_LABELS[w.role] || w.role}
                      </Badge>
                    )}
                  </div>
                </div>
            ...
            {allWorkers.length === 0 && (
              <p className="text-center text-slate-400 text-sm py-4">No staff or assigned clients found.</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle className="text-sm">Service Type Distribution</CardTitle></CardHeader>
        <CardContent>
