...
// focus: 'all' | 'plan' | 'closure' | 'followups' — workflow stages show the section they need.
// Closure of intensive case management does not end the participant's relationship with Candora.
export default function CaseTransitionTab({
  transition = {}, followups = [], onTransitionChange,
  onAddFollowup, onUpdateFollowup, onDeleteFollowup,
  meName, focus = 'all',
}) {
  ...
  const showPlan = focus === 'all' || focus === 'plan';
  const showClosure = focus === 'all' || focus === 'closure';
  const showFollowups = focus === 'all' || focus === 'followups';
      ...
      {showClosure && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">Documented rationale for closure</Label>
