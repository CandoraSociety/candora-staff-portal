...
export default function ELLLayout() {
  const location = useLocation();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  return (
    <ModuleGate moduleId="ell">
      <div className="min-h-screen bg-background">
        <header className="sticky top-0 z-40 border-b bg-sidebar">
          <div className="flex h-16 items-center justify-between px-6">
            <div className="flex items-center gap-3">
              <Link to="/ell" className="flex items-center gap-2">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-primary">
                  <GraduationCap className="h-5 w-5 text-primary-foreground" />
                </div>
                <div>
                  <h1 className="font-display font-bold text-sidebar-foreground">English Language Learning</h1>
                  <p className="text-xs text-sidebar-foreground/70">Program Manager</p>
                </div>
              </Link>
            </div>
