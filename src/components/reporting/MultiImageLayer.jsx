 ...
 * content area. Images use CSS float so they participate in the document
 * flow — text and tables wrap around them naturally instead of being
 * overlaid. Horizontal position (x) controls left/right float; vertical
 * position (y) controls margin-top.
 */
export default function MultiImageLayer({ section, onUpdate, branding, isPrint }) {
  ...
  return (
    <>
      {images.map(img => {
        const xVal = img.x != null ? img.x : 50;
        const floatSide = xVal < 50 ? 'left' : 'right';
        const widthVal = img.width || 40;
        return (
          <div
            key={img.id}
            ...
            style={{
              float: floatSide,
              width: `${widthVal}%`,
              marginTop: `${img.y || 0}px`,
              marginLeft: floatSide === 'right' ? '1rem' : '0',
              marginRight: floatSide === 'left' ? '1rem' : '0',
          ...
          >
            {editable && (
              <div className="no-print absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-30">
                <div
                  ...
                  title="Drag to reposition (horizontal = left/right, vertical = up/down)"
                >
            ...
            {editable && (
              <div className="no-print bg-black/55 rounded-b-lg px-2 py-1 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
