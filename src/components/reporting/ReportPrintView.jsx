...
function PagePreview({ pageNum, children, className, fitToPage }) {
  return (
    <div className={`relative print:shadow-none print:mb-0 mb-6 ${className || ''}`}>
      ...
      <div className="bg-white rounded-lg shadow-lg print:shadow-none print:rounded-none overflow-hidden" style={fitToPage ? { height: '11in', width: '210mm', maxWidth: '100%' } : { minHeight: '11in', width: '210mm', maxWidth: '100%' }}>
        {children}
      </div>
      {pageNum && (
    ...
    <div className="flex flex-col items-center gap-3">
      ...
      {sections.map((section, i) => (
        <PagePreview key={section.id} pageNum={getSectionPage(i)} fitToPage={section.fit_to_page}>
          ...
          <div className="p-8 relative" style={section.fit_to_page ? { height: 'calc(11in - 4px)', overflow: 'hidden' } : undefined}>
            <button
