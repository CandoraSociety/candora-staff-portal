...
export default function CollageRenderer({ photos, layout, isPrint }) {
  ...
  if (layout === 'strip') {
    return (
      <div className={`flex ${gap} rounded-lg overflow-hidden`}>
        {photos.map((url, i) => (
  ...
  if (layout === 'featured' && photos.length >= 2) {
    ...
    return (
      <div className={`rounded-lg overflow-hidden flex flex-col ${gap}`}>
  ...
  if (layout === 'mosaic' && photos.length >= 3) {
    ...
    return (
      <div className={`grid ${gap} rounded-lg overflow-hidden`} style={{ gridTemplateColumns: '1fr 1fr' }}>
  ...
  return (
    <div className={`grid ${gap} rounded-lg overflow-hidden`} style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
      {photos.map((url, i) => (
        <div key={i}>
