...
import { LayoutDashboard, Users, CalendarDays, Star, Menu, X, ArrowLeft, Monitor } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useOrgSettings } from '@/lib/useOrgSettings';
import EAFloatingWidget from '@/components/ed/EAFloatingWidget';

const NAV_ITEMS = [
  { path: '/digilit', label: 'Dashboard', icon: LayoutDashboard },
  { path: '/digilit/participants', label: 'Participants', icon: Users },
  { path: '/digilit/sessions', label: 'Sessions', icon: CalendarDays },
  { path: '/digilit/evaluations', label: 'Evaluations', icon: Star },
];

export default function DigiLitLayout() {
  const location = useLocation();
  const [mobileOpen, setMobileOpen] = useState(false);
  const { logoUrl, orgName } = useOrgSettings();

  const isActive = (path) => path === '/digilit' ? location.pathname === '/digilit' : location.pathname.startsWith(path);
      ...
      <header className="sticky top-0 z-40 bg-sidebar border-b border-sidebar-border">
        <div className="flex items-center justify-between px-4 lg:px-6 h-14">
          <div className="flex items-center gap-3">
            ...
            {logoUrl ? <img src={logoUrl} alt={orgName} className="h-8 w-auto object-contain" /> : <div className="h-8 w-8 rounded-lg bg-sidebar-primary flex items-center justify-center"><Monitor className="h-4 w-4 text-sidebar-primary-foreground" /></div>}
            <div><h1 className="text-sidebar-foreground font-display font-bold text-sm leading-tight">Digital Literacy</h1><p className="text-sidebar-foreground/60 text-xs leading-tight">In partnership with PALS</p></div>
          </div>
