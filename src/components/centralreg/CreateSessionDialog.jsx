...
// Date → duration → time-slot flow. Slots already booked in the selected room
// that day are shaded out; picking an available slot sets start & end times.
function ScheduleSection({ form, update, duration, setDuration, onValidity }) {
  return (
    <div className="col-span-2 space-y-2 border border-border rounded-md p-3 bg-muted/30">
      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Duration &amp; Time Slot</p>
      <TimeSlotPicker
        dateISO={form.session_date || ''}
        room={form.room || ''}
        duration={duration}
...
export default function CreateSessionDialog({ open, onOpenChange, area, program, onSaved }) {
  ...
  useEffect(() => {
    ...
    setDuration(60);
    setTimeOk(true);
    if (area === 'community') setForm({ session_date: todayStr(), status: 'scheduled', program_id: program?.id || '', program_name: program?.name || '', title: '', start_time: '', end_time: '', room: '', recurrence_pattern: 'none', recurrence_end_date: '' });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3">
          {(area === 'community' || area === 'phac') && (
            <>
              ...
              <ScheduleSection form={form} update={update} duration={duration} setDuration={setDuration} onValidity={setTimeOk} />
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving || !timeOk}>{saving ? 'Saving...' : `Create ${label}`}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
