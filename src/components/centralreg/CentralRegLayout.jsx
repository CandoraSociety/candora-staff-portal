...
export default function CentralRegLayout() {
      ...
      <header className="sticky top-0 z-40 bg-sidebar border-b border-sidebar-border">
        <div className="flex items-center justify-between px-4 lg:px-6 h-14">
          <div className="flex items-center gap-3">
            ...
            {logoUrl ? <img src={logoUrl} alt={orgName} className="h-8 w-auto object-contain" /> : <div className="h-8 w-8 rounded-lg bg-sidebar-primary flex items-center justify-center"><ClipboardCheck className="h-4 w-4 text-sidebar-primary-foreground" /></div>}
            <div><h1 className="text-sidebar-foreground font-display font-bold text-sm leading-tight">Central Registration</h1><p className="text-sidebar-foreground/60 text-xs leading-tight">One place to register for programs & volunteer</p></div>
          </div>
