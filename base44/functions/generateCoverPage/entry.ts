...
Style: Modern non-profit annual report cover. Elegant abstract patterns, gradients, or photography conveying community impact, growth, and hope. Portrait orientation. The design should be a clean 2D composition that fills the entire canvas — abstract background with the logo as focal point, title and year as accents.`,
      ...
      inside_front: custom_prompt || `${NO_MOCKUP}

Create a subtle inside front cover for a non-profit annual report as a flat 2D graphic design:
• Organization: ${branding.common_name || report.title}
• Colors: ${colors}

Style: Minimal, elegant inside cover design. This is what you see when you first open the report. Subtle brand color wash or very faint pattern. Mostly empty negative space with perhaps a small logo at the bottom or corner. Clean, quiet, understated. Portrait orientation.`,

      inside_back: custom_prompt || `${NO_MOCKUP}

Create a subtle inside back cover for a non-profit annual report as a flat 2D graphic design:
• Organization: ${branding.common_name || report.title}
• Colors: ${colors}

Style: Minimal, elegant inside back cover design. Subtle brand color wash or very faint pattern. Mostly empty negative space. Clean, quiet, understated. Portrait orientation.`,

      back: custom_prompt || `${NO_MOCKUP}
