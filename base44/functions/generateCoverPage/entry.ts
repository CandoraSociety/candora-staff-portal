...
Deno.serve(async (req) => {
  try {
    ...
    // For non-front covers, use the front cover as primary style reference for thematic consistency
    if (type !== 'front' && front_cover_url) referenceUrls.push(front_cover_url);
...
Style: Minimal, elegant inside cover design. This is what you see when you first open the report. Subtle brand color wash or very faint pattern. Mostly empty negative space with perhaps a small logo at the bottom or corner. Clean, quiet, understated. Portrait orientation.${addl}`,
...
Style: Minimal, elegant inside back cover design. Subtle brand color wash or very faint pattern. Mostly empty negative space. Clean, quiet, understated. Portrait orientation.${addl}`,
...
Style: Minimal, clean back cover design. Portrait orientation. Simple abstract background or subtle brand pattern. Light text for contact/address information if included.${branding.subsidiary_logos?.length ? ' Include space for funder/subsidiary logos at the bottom.' : ''}${addl}`
    };
